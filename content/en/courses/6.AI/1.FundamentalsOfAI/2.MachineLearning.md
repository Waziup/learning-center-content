...
In reinforced learning a learning argent learns from it's environment. So it works on interacting with the environment. The type of input data is not predefined. An agent rewards the model for success and does not reward it for a failure.
...
In recent years the neural networks are getting bigger and bigger, more layers, more parameters, ... . Also the size of the training data was increased. Therefore a new subfield of machine learning was found to host these kind of algorithms. It is called **Deep Learning**. With help of deep learning 
...
Ensemble learning combination of of different methods using numerous learning algorithms, to get better prediction results. The results are usually better than one approach alone. The models being trained independently and are combined afterwards. This is a lengthy process, because they have to be combined in a useful way.
...
Like stated before, there can be a combination of different machine learning models to give the best results

### 3. Extreme Feature Engineering

In extreme feature engineering the attribute decomposition and aggregation seen in data preparation is pushed to the limits.
